...
def mmd_unbiased(sample_p: torch.Tensor,
    ...
    distances = pdist(sample_pq, sample_pq, norm=2)
    sigma = 1 / (distances.median() ** 2)
...
def mmd_permutation_test(sample_p: torch.Tensor,
    ...
    distances = pdist(sample_p, sample_q, norm=2)
    sigma = 1 / (distances.median() ** 2)
...
def mmd_permutation_test_cc(sample_p1: torch.Tensor,
    ...
    distances = pdist(sample_p1, sample_q, norm=2)
    sigma = 1 / (distances.median() ** 2)
    est = mmd_unbiased(sample_p1, sample_q, sigma)
    # data = []
    mmd_vals = torch.zeros(n_permutations)
    for i in range(n_permutations):
